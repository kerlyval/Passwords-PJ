...
class UserService {
    constructor(emailService) {
        ...
        this.sendEmailValidationLink = (email) => __awaiter(this, void 0, void 0, function* () {
            const token = yield jwt_adapter_1.JwtAdapter.generateToken({ email }, '300s'); //indica que durara 5 mins
            if (!token)
    ...
    <a href="${link}">Validate your email:${email}<a>
     `;
        ...
        this.validateEmail = (token) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (!user)
                throw domain_1.CustomError.internalServer('Email does not exist');
            user.status = data_1.Status.ACTIVE;
            try {
                yield user.save();
                return {
                    menssage: 'User activated',
                };
            }
            catch (error) {
    ...
    login(credentials) {
        return __awaiter(this, void 0, void 0, function* () {
            //1. buscar el usuario que se requiere loguear
            ...
            //3. debemos genera un jwt
            const token = yield jwt_adapter_1.JwtAdapter.generateToken({ id: user.id }); //!Importante: Recordar que el payload del JWT viene con el ID del Usuario que se esta logeando
            if (!token)
                throw domain_1.CustomError.internalServer('Error creating the JWL');
            //4. enviar la dara al cliente
            return {
                token: token,
                user: {
                    id: user.id,
    ...
    register(userData, file) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (error.code === '23505') {
                    throw domain_1.CustomError.badRequest(`User with email: ${userData.email}  already exists`);
                }
                throw domain_1.CustomError.internalServer('Error creating user');
            }
            // enviar un correo electronico de verificación de cuenta
        });
    }
